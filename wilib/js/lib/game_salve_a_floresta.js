...
var bg = new Image();
bg.src = '../img/fotos/45/bg2.png';

var smoke = new Image();
smoke.src = '../img/fotos/45/smoke.png';

var plant = new Image();
plant.src = '../img/fotos/45/plant.png';
...
function update(dt) {
  ...
  if
  (
    score == 500 && plants.length < 3 ||
    score == 1000 && plants.length < 6 ||
    score == 1500 && plants.length < 12
  ) {
    plants.push({
      pos: [Math.random() * (canvas.width - 70), 310],
      sprite: new Sprite(plant, [0, 0], [50, 46], 0, [0])
    });
  }
